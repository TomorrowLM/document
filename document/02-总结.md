...
- 布局 （Layout/自动重排 Reflow）：**负责计算各个元素节点的尺寸、位置**。基于页面的流式布局，遍历渲染树节点，不断计算节点最终的位置，几何信息，样式等属性后，输出一个“盒模型”

- 绘制（Painting）：将节点大小，位置根据屏幕的窗口大小换算成真实的像素，颜色等属性绘制到图层上
...
文档流是文档中可显示对象在排列时占用的位置/空间。例如：[块元素](https://so.csdn.net/so/search?q=块元素&spm=1001.2101.3001.7020)自上而下摆放，内联元素从左到右摆放。（文档流中限制非常的多，导致很多页面效果无法实现)。
