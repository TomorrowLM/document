...
postMessage 是 JavaScript 中用于实现在不同窗口、iframe、Web Worker或者标签页之间之间通信的方法
...
比如攻击者在论坛中加一个恶意表单， 当用户提交表单的时候，却把信息传送到攻击者的服务器中，而不是用户原本以为
...
CSRF攻击是用户在已登录状态下访问黑客的链接，链接里面会构造一个表单，表单的地址为目标网站的接口地址，浏览器会自动以用户的身份发送请求，伪造用户请求。
...
文档流（Document Flow）是网页布局中的一个核心概念，**描述了元素在页面上默认的排列规则**。例如：[块元素](https://so.csdn.net/so/search?q=块元素&spm=1001.2101.3001.7020)自上而下摆放，内联元素从左到右摆放。（文档流中限制非常的多，导致很多页面效果无法实现)。
