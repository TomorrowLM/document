...
组件封装不是简单地“把功能写进去”，而是遵循一套明确的结构和设计原则，以确保组件高内聚、低耦合。

- 组件应该实现什么功能
- 组件的外观和布局，是否需要slot去扩展
- 接收哪些外部输入（props）、触发哪些事件（events）并将数据传给父组件
- 组件不处理数据的存储、不与外部业务耦合
  - 举例：vue3的usetable的hooks
...
- 严格模式式：又称标准模式，是指浏览器按照 W3C 标准解析代码。
  - **严格模式的限制**
    - 变量必须声明后再使用
    - 函数的参数不能有同名属性，否则报错
    - 不能使用with语句
...
- 案例二：
  ...
  ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/7c1ca9857f8dad97bbc54df2678bfc50.png)

  

  ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/37c169be0552fb360c64ab0f9f08d2f8.png)

  但是我们看到的却是红色的div距离顶部有个20px的距离，这其实涉及到了外边距重叠的问题了，那么要想解决这个问题，就是给 红色的div 开启bfc，让它成为有个独立的“区域”，这边我演示一下
  ![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a77a7e0e7bfac56fb0a3bb5f6cc9e2e0.png)![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/bf7f3296bba14de72a76d8459b4244c0.png)
...
```html
html 体验AI代码助手 代码解读复制代码element.scrollIntoView(); // 等同于element.scrollIntoView(true)
