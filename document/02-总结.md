...
- `Render`渲染进程的`Renderer`接口收到消息，简单解释后，交给渲染线程`GUI`，绘制Render树
- 当然可能会有`JS`线程操作`DOM`（这可能会造成回流并重绘）
- 最后`Render`渲染进程将结果传递给`Browser`主进程
- `Browser`主进程接收到结果并将结果`GPU`进程绘制出来
...
`Event Loop`即事件循环，是指浏览器或`Node`的一种**解决`javaScript`单线程运行时阻塞的**一种机制，也就是我们经常使用**异步**的原理。
...
![img](https://upload-images.jianshu.io/upload_images/7567844-72f2b2c531f65689.png?imageMogr2/auto-orient/strip|imageView2/2/w/886/format/webp)
...
这是一种用于发送表单数据的类型，它会将数据以键值对的形式编码，例如`name=Tom&age=18`。键和值都会进行URL编码，以避免特殊字符的影响。

这种类型的优点是可以发送任何类型的字符，所有浏览器都支持。但缺点是不能发送二进制数据，如文件上传，而且URL编码会增加数据的长度，可能会超过服务器的限制。
...
跨域资源共享([CORS](https://link.zhihu.com/?target=https%3A//developer.mozilla.org/zh-CN/docs/Glossary/CORS)) 是一种基于 [HTTP](https://developer.mozilla.org/zh-CN/docs/Glossary/HTTP) 头的机制，**CORS 需要浏览器和服务器同时支持**。

- 浏览器使用额外的HTTP头来告诉浏览器，让运行在一个 origin (domain) 上的 Web 应用被准许访问来自不同源服务器上的指定的资源。

- 目前，所有浏览器都支持该功能。因此，**实现 CORS 的关键是服务器**。**服务器端对于CORS的支持**，主要就是通过**设置Access-Control-Allow-Origin**来进行的。
...
```
复制代码var xhr = new XMLHttpRequest(); // IE8/9需用window.XDomainRequest兼容
...
"`document.domain`是一个JavaScript属性，它用于设置或获取当前文档的域名。它的作用是允许同源策略下的**不同子域(例如，在域名“example.com”下，“mail.example.com”和“calendar.example.com”都是子域)**之间进行通信，从而实现跨域操作。

`document.domain`的限制主要包括以下几点：

1. `document.domain`只能设置为当前文档的顶级域名或者其子域名。例如，如果当前文档的域名是`example.com`，那么`document.domain`可以设置为`example.com`或者其子域名如`subdomain.example.com`，但不能设置为其他域名如`google.com`。
2. `document.domain`必须在所有相关页面的脚本中设置为相同的值。这意味着，如果一个页面将`document.domain`设置为`example.com`，那么其他与之进行跨域通信的页面也必须将`document.domain`设置为`example.com`，否则跨域通信将被浏览器阻止。
3. `document.domain`只对使用相同协议和端口的页面起作用。例如，如果一个页面使用`http://example.com`访问，那么其他同源的页面也必须使用`http://example.com`，而不能使用`https://example.com`或者`http://subdomain.example.com`。
...
示例：https://juejin.cn/post/7074217735928381448
...
- ③所有`引用类型的__proto__`属性`指向构造函数的prototype`
...
- defer是在html渲染完之后，DomCOntentLoaded之前执行，所以不会阻塞后续html的解析。

  如果你的脚本代码依赖于页面中的`DOM`元素（文档是否解析完毕），或者被其他脚本文件依赖。

- async加载完后会立即执行,执行的过程仍会阻塞后续html的解析
