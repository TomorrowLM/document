...
- ‌**file-loader**‌适用于需要确保文件路径正确且文件较大的情况。它通过复制文件到指定目录并修改引用路径来解决路径问题，适合于需要确保资源路径正确性的场景
...
- **Prefetch**，预请求，是为了提示浏览器，用户**未来的浏览**有可能需要加载目标资源，所以浏览器有可能通过事先获取和缓存对应资源，优化用户体验。
...
需要注意的是，**`Preload和Prefetch`都不是浏览器强制加载资源，而是给浏览器提供了一些提示，让它在空闲时间主动加载一些资源，从而提升应用程序的性能体验**。这两项技术它们的**兼容性不是很好**
