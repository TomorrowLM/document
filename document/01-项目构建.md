...
commonjs模块对commonjs模块的导入，或者es模块对es模块的导入，而上面的代码就是es模块对commonjs模块的导入的解决方法。

源码：会判断module是否为es模块，当`__esModule`为true的时候，标识module为es模块，那么`module.a`默认返回`module.default`，否则返回`module`
...
TomorrowLM: 扩展'ESLint'配置为格式化程序，但不能格式化'vue'-文件
...
确保你已经安装了Volar扩展，可以通过以下命令搜索并安装：

1. **Search marketplace** - 搜索并安装Volar扩展。
...
主要用于紧急修复一些 bug。**会从 master 分支上建立**，修复结束后会再合并到 dev 和 release 分支上
