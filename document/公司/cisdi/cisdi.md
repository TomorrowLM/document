...
实时数据采集是赛迪信息打造的实时数据库下的实时数据采集产品，该产品主要**实现了实时数据从各种源头进行采集、整合并存储到实时数据库中的过程**。这些源头可以来自各种类型的传感器、设备、机器人，甚至其他数据库和采集器。

痛点：

- 市场上时序数据来源、数据格式以及采集协议多种多样
- 数据分散存储难以满足多种场景下数据复合使用。
- 部分第三方系统不具备采集实时数据能力。

价值：内置多种通用工业标准协议+自定义协议实现统一采集。

目的是：通过研发支持多种主流工业协议的通用框架，屏蔽各类工业设备和系统的异构性，并通过自研工业协议，提供统一的数据格式和高效的数据读写。替代传统分散存储，统一存储。按标准格式给外部系统和应用程序提供数据服务
...
| 点位      | **用于标识和管理设备的数据点，每个数据点代表设备的状态或相关参数。点位地址即点位的驱动程序地址** |
...
![9f978fe267d266cdc5f0d8834391229b](../../img/公司/9f978fe267d266cdc5f0d8834391229b.png)

实时数据采集流程

- 操作流程如下：

  1、新增连接，关联采集所需协议，同时填写**协议，连接域名**相关参数。
...
![3008c8e1f1358b20d6c9adcf21834d91](../../img/公司/3008c8e1f1358b20d6c9adcf21834d91.png)
...
对时序数据进行加工处理的项目。首先添加数据源，然后通过添加元素并绑定元素的刷新时间，元素下也可以添加相应的属性以及属性的规则（即通过代码实现的规则）。当元素被触发时，就会执行属性的规则，若满足规则就会往数据中添加一条数据

> 传感器就是数据源，温度就是元素，温度大于100就是一条属性
...
通过在main.ts调用Vue.use执行自定义的install方法，先用require.context获取所有文件组件，在用vue.component注册全局组件。

需要了解一个概念，页面是当前展示内容的集合，内容是由组件构成。所以所有的组件信息都会放在页面的配置项中

我们先通过获取所有组件下的schema.json配置并保存下来。然后当拖拽一个组件的时候会拿到相应组件的配置并放在页面配置的widgetList里面。

然后通过component标签循环渲染widgetList下的组件，并传入相应的配置信息

然后就是定义组件，组件都是嵌入在item-widget组件下的，这样方便配置宽度和自适应。然后会用mixins混入公共的属性方法
